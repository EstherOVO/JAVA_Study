...
/*
   ...
   - 순서를 유지해야 하는 로그 정보 등에 쓰인다.
   - 삽입 순서를 기억해야 할 필요가 있을 경우
*/
    public static void main(String[] args) {
//      Map 인터페이스 추상화를 통해 필요한 자료 구조로 변경
//      성능 : HashMap과 비슷한 성능을 가지지만, 순서 정보를 유지하는 데 간접비용(Overhead)가 발생한다.
        ...
        linkedHashMap.put("링크드 리스트라서", 3);
        linkedHashMap.put("순서가 보장돼요", 4);
...
//      LRU(Least Recently Used) Cache : 가장 최근에 접속한 순서에 따라 순서 요소 재배치
//      LinkedHashMap 생성자에서 accessOrder를 true로 설정할 경우
        ...
        lruCache.get("한글문서2");  // 최근에 사용한(조회만 해도) 접근 요소가 마지막으로 옮겨짐
        lruCache.get("이미지1");
