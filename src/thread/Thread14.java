...
//  커피 컵을 소비
    public synchronized void takeOut(Customer customer) {
        while (cups <= 0) {
            System.out.println(String.format("[컵 : %d] %s : 커피가 없어요.", cups, customer.name));
            try {
                wait(); // 버퍼가 비어있으면 대기
...
//      버퍼를 1개 줄임
        System.out.println(String.format("[컵 : %d] %s : 커피 테이크아웃 합니다~", cups, customer.name));
        cups--;
        notifyAll();    // 모두에게 알린다.
...
//      커피 컵이 여유가 있으면 대기
        while (cups > 3) {
            System.out.println(String.format("[컵 : %d] : 매니저 / 재고 여유 있습니다.", cups));
