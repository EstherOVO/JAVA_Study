 ...
 */
        ...
        Thread thread1 = new Thread(itemShop::buyItem, "희동");
    ...
    public static class ItemShop  {
        ...
        public synchronized void buyItem() {
            ...
            if (itemAvailable) {
                System.out.println(String.format("플레이어 [%s]님이 아이템을 성공적으로 구매했습니다.", Thread.currentThread().getName()));
                itemAvailable = false;
            } else {
                System.out.println(String.format("플레이어 [%s]님의 구매 시도 실패 : 아이템이 이미 판매되었습니다.", Thread.currentThread().getName()));
            }
        }
    }
}
